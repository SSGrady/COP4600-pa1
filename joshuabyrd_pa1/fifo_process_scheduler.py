    ...
    while time < run_for:
        ...
        if current_process:
            if time == current_process.start_time + current_process.burst:
                ...
                # If there are processes in the queue and the current process has finished
...
def write_output(filename, processes, events, run_for):
    ...
    with open(output_file, 'w') as file:
        ...
        for process in processes:
            file.write(
                f"{process.name} wait {process.wait_time} turnaround {process.turnaround_time} response {process.response_time}\n")
    ...
    try:
        with open(input_file, 'r') as file:
            ...
            if len(processes) != process_count:
                print("Error: Number of processes does not match the specified count")
                sys.exit(1)
